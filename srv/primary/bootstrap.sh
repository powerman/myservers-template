...
#-- DigitalOcean: 1 vCPU, 1GB RAM and 25GB disk.
...
# Add more RAM (swap + zswap).
fallocate -l 2G /swapfile
chmod 600 /swapfile
mkswap /swapfile
swapon /swapfile
echo '/swapfile none swap sw 0 0' >>/etc/fstab
# zswap compresses swap pages in RAM before writing to disk (lz4, ~2-3x ratio).
# z3fold packs up to 3 compressed pages per physical page (75% density vs zbud's 50%).
# max_pool_percent=25 allows ~245MB of RAM for compressed pool on 1GB server.
echo 1 >/sys/module/zswap/parameters/enabled
echo lz4 >/sys/module/zswap/parameters/compressor
# z3fold must be in initramfs — zswap initializes before module autoloading.
echo z3fold >>/etc/initramfs-tools/modules
echo 'GRUB_CMDLINE_LINUX_DEFAULT="console=tty1 console=ttyS0 net.ifnames=0 biosdevname=0 zswap.enabled=1 zswap.compressor=lz4 zswap.zpool=z3fold zswap.max_pool_percent=25"' >/etc/default/grub.d/99-zswap.cfg
update-grub
update-initramfs -u
